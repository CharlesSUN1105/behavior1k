...
class Visual:
    ...
    def __init__(
        self,
        offset: Optional[sc.Transformation] = None,
        geom_type: Optional[str] = None,
        geom_param: Any = None,
    ) -> None:
        self.offset = offset or sc.Transformation.identity()
...
class Joint:
    ...
    def __init__(
        ...
        if self.joint_type != "fixed" and axis is None:
            self.axis = cs.DM([0.0, 0.0, 1.0])
        else:
            self.axis = cs.DM(axis) if axis is not None else cs.DM([0.0, 0.0, 1.0])
        self.limit = limit
...
class Frame:
    ...
    def _ptree(self, indent_width: int = 4) -> str:
        def _inner_ptree(
            ...
            for child in parent.children[:-1]:
                ...
                show_str += _inner_ptree(
                    root, child, parent, indent + "│" + " " * (indent_width + 1)
                )
            if parent.children:
                child = parent.children[-1]
                show_str += indent + "└" + "─" * indent_width
                show_str += _inner_ptree(
                    root, child, parent, indent + " " * (indent_width + 2)
                )
            return show_str
    ...
    def get_transform(self, theta: float = 0.0) -> sc.Transformation:
        if self.joint.joint_type == "revolute":
            rotvec = theta * self.joint.axis
            r = sc.Rotation.from_rotvec(rotvec)
            t = sc.Transformation(r, sc.Translation.identity())
        elif self.joint.joint_type == "prismatic":
            t = sc.Transformation(sc.Rotation.identity(), sc.Translation(theta * self.joint.axis))

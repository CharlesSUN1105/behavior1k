...
class Goal:
    ...
    def get_position(self, t):
        ...
        y = 0.5 * np.sin(2.0 * np.pi * 0.5 * t)
        z = 0.2 * np.sin(2 * np.pi * 0.1 * t)
        return np.array([0, y, self.sign1 * z])
...
def main():
    diffik = WholeBodyDifferentialIK(PYBULLET_TIME_STEP)
    ...
    video = start_recording_video(client, "diff_ik.mp4")

    unit_quat = (0, 0, 0, 1)

    def jac_left():
        q = get_q(client, r1pro, diffik.left_arm_chain.get_joint_parameter_names())
        return diffik.left_arm_chain.jacobian(q).toarray()
    ...
    try:
        ...
        while True:
            ...
            vel_goal_data.append(goal_left(t)[:3].tolist())

            config = {
                "vG_left": goal_left(t),
                "vG_right": goal_right(t),
                "w_left": w_left,
                "w_right": w_right,
                "w_dq": w_dq,
                "q": q,
            ...
            vel_act_data.append(v_act[:3].tolist())

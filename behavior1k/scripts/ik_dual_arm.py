...
def sample_collision_spheres(chain: Chain, q0, nspheres: int, max_attempts=10_000):
...
def main():
    ...
    sphere_radius, sphere_positions = sample_collision_spheres(
        planner.chain, r1pro_init_joint_state, nspheres
    )

    joint_limit_safety = 0.99
    ik = GlobalDualArmIK(
        joint_limit_safety=joint_limit_safety,
        collision_sphere_names=collision_sphere_names,
    )

    pG_left = [0.7, 0.1, 1.2]
    pG_right = [0.7, -0.1, 1.0]
    ...
    # Execute plan
    video = start_recording_video(client, "ik_dual_arm.mp4")
    t = 0.0
    angle = 0.0
